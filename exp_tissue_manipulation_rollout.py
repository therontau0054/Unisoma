...
from data_loaders.tissue_manipulation import DataLoader_Tissue_Manipulation_Rollout
...
def train(data_path, net, config, normalizers, criterion):
    ...
    train_data_loader = DataLoader_Tissue_Manipulation_Rollout(
        data_path = data_path,
        mode = "train"
    )

    eval_data_loader = DataLoader_Tissue_Manipulation_Rollout(
        data_path = data_path,
        mode = "eval"
    )
    ...
    for epoch in range(epochs):
        ...
        for i, train_data in enumerate(train_datas):
            ...
            for b in range(0, total_batch, batch_size):
                ...
                print(f"epoch: {epoch}, lr: {scheduler.get_last_lr()[0]}, sample {i}, batch: {b}, train_loss: {loss.data}")
        ...
        with torch.no_grad():
            for i, eval_data in enumerate(eval_datas):
                ...
                for b in range(total_batch):
                    ...
                    loss = sum(losses)
                    print(f"epoch: {epoch}, sample: {i}, frame: {b}, eval_loss: {loss.data}")
                    mean_eval_trajectory_loss += loss.data
                
                mean_eval_trajectory_loss /= total_batch
                mean_eval_loss += mean_eval_trajectory_loss
                print(f"epoch: {epoch}, sample: {i}, mean_eval_trajectory_loss: {mean_eval_trajectory_loss}")
...
def test(data_path, net, config, normalizers, criterion):
    ret_save_path = f"{data_root}/ret/{model_name}_tissue_manipulation_rollout_{version}.pkl"
    test_data_loader = DataLoader_Tissue_Manipulation_Rollout(
        data_path = data_path,
        mode = "test"
    )
    ...
    with torch.no_grad():
        ...
        for i, test_data in enumerate(test_datas):
            ...
            for b in range(total_batch):
                ...
                losses = [criterion(pred_pos[j], _next_deformable[j], use_sqrt = True) for j in range(len(pred_pos))]
                loss = sum(losses)
                print(f"sample: {i}, frame: {b}, test_loss: {loss.data}")
            ...
            print(f"sample: {i}, mean_test_trajectory_loss: {mean_test_trajectory_loss}")
...
if __name__ == "__main__":
    config_path = "./configs/tissue_manipulation.toml"
    ...
    # setsid python -u exp_tissue_manipulation_rollout.py &