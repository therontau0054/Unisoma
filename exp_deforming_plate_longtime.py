...
from data_loaders.deforming_plate import DataLoader_Deforming_Plate_LongTime
...
def train(data_path, net, config, normalizers, criterion):
    ...
    train_data_loader = DataLoader_Deforming_Plate_LongTime(
        data_path = data_path,
        mode = "train"
    )

    eval_data_loader = DataLoader_Deforming_Plate_LongTime(
        data_path = data_path,
        mode = "eval"
    )
    ...
    for epoch in range(epochs):
        ...
        for i, train_data in enumerate(train_datas):
            ...
            stress_losses = [criterion(outputs[j][:, :, 3:], target_deformable[j][:, :, 3:]) for j in range(len(target_deformable))]
            pos_loss = sum(pos_losses)
            stress_loss = sum(stress_losses)
            loss = pos_loss + stress_loss
            print(f"epoch: {epoch}, lr: {scheduler.get_last_lr()[0]}, batch: {i}, train_loss: {loss.data}, train_pos_loss: {pos_loss.data}, train_stress_loss: {stress_loss.data}")
            mean_train_loss += loss.data
            mean_train_pos_loss += pos_loss.data
            mean_train_stress_loss += stress_loss.data
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            scheduler.step()
        mean_train_loss /= len(train_datas)
        mean_train_pos_loss /= len(train_datas)
        mean_train_stress_loss /= len(train_datas)
        print(f"epoch: {epoch}, mean_train_loss: {mean_train_loss}", f"mean_train_pos_loss: {mean_train_pos_loss}", f"mean_train_stress_loss: {mean_train_stress_loss}")
        if (epoch + 1) % eval_per_epoch != 0: continue
        net.eval()
        mean_eval_loss = 0.
        mean_eval_pos_loss = 0.
        mean_eval_stress_loss = 0.
        with torch.no_grad():
            for i, eval_data in enumerate(eval_datas):
                ...
                stress_losses = [criterion(outputs[j][:, :, 3:], final_deformable[j][:, :, 3:]) for j in range(len(final_deformable))]
                pos_loss = sum(pos_losses)
                stress_loss = sum(stress_losses)
                loss = pos_loss + stress_loss

                print(f"epoch: {epoch}, batch: {i}, eval_loss: {loss.data}, eval_pos_loss: {pos_loss.data}, eval_stress_loss: {stress_loss.data}")
                mean_eval_loss += loss.data
                mean_eval_pos_loss += pos_loss
                mean_eval_stress_loss += stress_loss

            mean_eval_loss /= len(eval_datas)
            mean_eval_pos_loss /= len(eval_datas)
            mean_eval_stress_loss /= len(eval_datas)

            print(f"epoch: {epoch}, mean_eval_loss: {mean_eval_loss}, mean_eval_pos_loss: {mean_eval_pos_loss}, mean_eval_stress_loss: {mean_eval_stress_loss}")
...
def test(data_path, net, config, normalizers, criterion):
    ret_save_path = f"{data_root}/ret/{model_name}_deforming_plate_longtime_{version}.pkl"
    test_data_loader = DataLoader_Deforming_Plate_LongTime(
        data_path = data_path,
        mode = "test"
    )
    ...
    mean_test_loss = 0.
    mean_test_pos_loss = 0.
    mean_test_stress_loss = 0.
    with torch.no_grad():
        ret_save_data = {}
        for i, test_data in enumerate(test_datas):
            ...
            stress_loss = sum(stress_losses)
            loss = pos_loss + stress_loss
            print(f"batch: {i}, test_loss: {loss.data}, test_pos_loss: {pos_loss.data}, test_stress_loss: {stress_loss.data}")

            ret_save_data[i]["pred_pos"] = [p.cpu().numpy().copy() for p in pred_pos]
            mean_test_loss += loss
            mean_test_pos_loss += pos_loss
            mean_test_stress_loss += stress_loss
        mean_test_loss /= len(test_datas)
        mean_test_pos_loss /= len(test_datas)
        mean_test_stress_loss /= len(test_datas)
        print(f"mean_test_loss: {mean_test_loss}, mean_test_pos_loss: {mean_test_pos_loss}, mean_test_stress_loss: {mean_test_stress_loss}")
...
if __name__ == "__main__":
    config_path = "./configs/deforming_plate.toml"
    ...
    # setsid python -u exp_deforming_plate_longtime.py &